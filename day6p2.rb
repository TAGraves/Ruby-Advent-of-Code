...
i = 0
$grid = {}
while i < 1000 #create light $grid
  e = 0
  while e < 1000
    $grid[[i,e].clone] = 0
    e += 1
...
def toggle_light(beginningRange, endingRange)
  start = beginningRange.split(",")
  finish = endingRange.split(",")
  start[0] = start[0].to_i
  start[1] = start[1].to_i
  finish[0] = finish[0].to_i
  finish[1] = finish[1].to_i
  while start[0] < finish[0]+1
    iterate = start[1]
    while iterate < finish[1] + 1
      $grid[[start[0],iterate].clone] += 2
      iterate += 1
    end
    start[0] += 1
  end
...
def turn_off_light(beginningRange, endingRange)
  start = beginningRange.split(",")
  finish = endingRange.split(",")
  start[0] = start[0].to_i
  start[1] = start[1].to_i
  ...
  while start[0] < finish[0]+1
    iterate = start[1]
    while iterate < finish[1] + 1
      $grid[[start[0],iterate].clone] += -1
      if $grid[[start[0],iterate].clone] < 0
        $grid[[start[0],iterate].clone] = 0
      end
      iterate += 1
    end
    start[0] += 1
...
def turn_on_light(beginningRange, endingRange)
  start = beginningRange.split(",")
  finish = endingRange.split(",")
  start[0] = start[0].to_i
  start[1] = start[1].to_i
...
def process_instructions(instructions)
  ...
  for instruction in instructions
    instructionType = instruction.split(" ")
    case instructionType[0]
    when "toggle" 
      toggle_light(instructionType[1], instructionType[3])
    when "turn"
      if instructionType[1] == "off"
        turn_off_light(instructionType[2], instructionType[4])
      else
        turn_on_light(instructionType[2], instructionType[4])
      end
    end
  end
end
...
#TEST3 = "turn on 0,0 through 999,999
