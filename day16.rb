def parse_machine(mfcsam)
  qualities = {}
  mfcsam.each do |quality|
    matcher = quality.match(/(\w+): (\d+)/)
    qualities[matcher[1]] = matcher[2].to_i
  end
  return qualities
end

def parse_aunts(aunts)
  aunts_list = []
  aunts.each do |aunt|
    matcher = aunt.scan(/(\w+): (\d+)/)
    auntIndex = aunt.match(/Sue (\d+)/)
    auntie = {"num" => auntIndex[1].to_i}
    matcher.each do |match|
      auntie[match[0]] = match[1].to_i
    end
    aunts_list.push(auntie)
  end
  return aunts_list
end

def check_aunt(aunt, qualities)
  aunt.each do |characteristic, value|
    if characteristic != "num" && qualities[characteristic] != value
      return false
    end
  end
  return true
...
AUNTS = "Sue 1: goldfish: 9, cars: 0, samoyeds: 9
...
Sue 500: cats: 2, goldfish: 9, children: 8".split("\n")

puts find_aunt(MFCSAM, AUNTS)